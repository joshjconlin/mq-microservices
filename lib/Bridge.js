...
* @param config {
    performAuth: boolean;
*   rmqConfig: object; // if null mqUrl will be used
    mqUrl: string;
*   queues: {
*       process: string;
*       success: string;
...
class Bridge {
   ...
   * */
    ...
    * */
    ...
    _getConnectConfiguration = () => {
        const shouldAuth = this.config.performAuth ?? false;
        if (shouldAuth) {
            return this.config.rmqConfig;
        }
        return this.config.mqUrl;
    };

    _connect = async () => {
        return new Promise((resolve, reject) => {
            console.log('Starting connection...');
            const connectParams = this._getConnectConfiguration();

            amqp.connect(connectParams, (error, connection) => {
                if (error) {
    ...
    _startServerless = () => {
        const child = spawn(this.config.startCommand ? this.config.startCommand : 'serverless offline', { stdio: 'inherit', shell: true });
